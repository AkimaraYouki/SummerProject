...
**남은 문제는 지오메트리 완전성**: 보드가 빠진 채로 임포트되면 전자보드의 시각적/충돌
메시가 로봇에서 통째로 빠진다. 물리 시뮬레이션(질량·관성) 자체엔 영향 없지만, USD로
변환한 뒤 Isaac Sim에서 트렁크 안에 보드가 보이는지 매번 확인이 필요하다.

**근본 해결책**: OnShape에서 보드 서브어셈블리를 트렁크(또는 실제 장착되는 곳)에 메이트로
연결해서 단일 연결 트리로 만들 것. 그 전까지는 임포트할 때마다 로그에 `board` 문자열이
있는지(`grep -i board <로그파일>`) 확인해서 이번 임포트에 보드가 포함됐는지 매번 체크하는
걸 권장.
