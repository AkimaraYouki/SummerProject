...
## 2026-09-17 — 실물 사양으로 다시 맞추고, 심 무게중심 방향 세 가지를 학습했다

### 실기에서 드러난 것

1. **v73 ~ v82 는 전부 실물에 없는 발로 학습됐다.** 이 판들은 big_foot USD 를
   썼는데, 그때 실물에는 원래 발이 달려 있었다. v75 가 심에서는 모든 지표가
   좋았는데 실기에서 v65 보다 못했던 이유 중 하나다. 원래 발로 다시 학습한 것이
   v83 이다 (12000 iter 까지 학습, 측정은 아직).
2. **그 뒤 실물을 big_foot 으로 바꿨다.** 이제부터 기준 모델은 big_foot 이다.
3. **실측 총질량은 2894 g** 으로, CAD 값보다 142 g 무겁다. `big_foot/robot_m2894.urdf`
   는 몸통(trunk_assembly) 질량만 1.153 -> 1.295 kg 으로 바꾸고 관성은 같은 비율
   (x1.123)로 키운 것이다. **CoM 위치는 그대로 뒀다.** 이 URDF 로 v84 를 시작했지만
   500 iter 에서 멈췄다.
4. **실물은 전부 앞으로 넘어진다.** 사용자가 걷는 로봇의 뒷부분을 손으로 눌러
   주자 잘 걸었다. 실물이 정책이 기대하는 것보다 앞쪽이 무겁다는 뜻이다.

### 재학습 없이 쓰는 카드: `rl_walk.py --lean-back DEG`

IMU 값(gyro / accel / 중력 방향)을 R_y(deg) 로 돌린 뒤 정책에 넣는다. 그러면 정책은
몸이 앞으로 기울었다고 믿고 뒤로 젖혀서 바로잡는다. 1° 는 전체 무게중심 약 1 mm 에
해당한다 (상체 1908 g, 고관절 위 87 mm 기준). 부호는 `tests/test_lean_back_sign.py`
로 고정했다. 기본값은 0 이다.

### v85 / v86 / v87 — v65 에서 분기

v65 에서 분기한 이유는 실기에서 가장 잘 걸은 정책이기 때문이다. 세 판 모두 로봇은
현재 실물 사양(big_foot + 2894 g)이고, 다른 점은 심 몸통 CoM 의 x 이동 하나뿐이다.

    v85   뒤로 15 mm   (사용자 가설: 뒤가 무거운 조건에서 학습)
    v86   앞으로 15 mm (반대 가설: 실물처럼 앞이 무거운 조건에서 학습)
    v87   이동 없음    (기준)

몸통 15 mm 는 전체 무게중심으로 약 6.7 mm 다. 세 판 모두 3000 iter, 512-256-128.

**측정** — 판마다 3 회 측정했다. 칸의 값은 평균이고 괄호 안은 최소 ~ 최대다.

                 v87 (기준)            v85 (뒤 15)           v86 (앞 15)
    일률 W       9.84 (9.67~9.93)      10.16 (9.98~10.39)    10.77 (10.72~10.79)
    포화 %       1.32 (1.28~1.40)      1.16 (1.07~1.26)      1.52 (1.49~1.57)
    rollRMS °    4.85 (3.82~6.14)      4.23 (4.00~4.42)      4.90 (4.27~5.38)
    roll 속      84.6 (66~112)         71.1 (65~77)          88.0 (64~104)
    낙상 %       0.73 (0.13~1.38)      0.65 (0.19~1.25)      0.90 (0.75~1.00)
    전후 추종    0.0160                0.0156                0.0160
    회전 추종    0.0424 (0.026~0.063)  0.0222 (0.008~0.032)  0.0495 (0.032~0.072)
    횡 추종      0.0377 (0.033~0.048)  0.0263 (0.022~0.029)  0.0310 (0.027~0.034)
    6 방향 부호  OK                    OK                    OK

기준(v87)과 비교해 **범위가 겹치지 않는 차이**만 추리면 다음과 같다.

- v85: 포화 감소, 횡 추종 개선, 일률 소폭 증가
- v86: 포화 증가, 일률 증가

ONNX 변환 검증 오차는 v85 5.96e-07, v86 2.98e-07, v87 2.98e-07 이다. 6 방향 녹화는
`~/odm_out/video/v8{5,6,7}_6dir.mp4` 에 있다.

**이 표로는 방향을 정할 수 없다.** 세 정책은 각자 학습한 무게중심 조건에서 측정됐다.
v86 은 앞이 무거운 로봇을 지탱하니 토크를 더 쓰는 게 당연하다. 이 표는 각 심 환경이
얼마나 어려운지를 보여 줄 뿐, 실물에 올렸을 때 어느 쪽이 잘 걷는지는 말해 주지
않는다. v75 가 이미 이 함정을 보여 줬다 (심 1 등, 실기에서는 v65 보다 못함).

판정은 실기에서 한다. 조건은 세 판 모두 같게 한다 — LPF 없음, `--lean-back` 없음.

- v85 가 가장 잘 걸으면: 사용자 가설이 맞다
- v86 이 가장 잘 걸으면: 심 CoM 을 실물처럼 앞에 두는 게 맞다
- v87 이 가장 잘 걸으면: 무게중심은 원인이 아니었고, 실물 사양(발·질량)을 맞춘
  것으로 충분했다

참고: 08-18 의 `_ComBackEventCfg` 독스트링은 "실물 CoM 이 심보다 앞에 있다"고
추론하면서도 구현은 심 CoM 을 뒤로 옮겼다. v70 이후(뒤 10 mm) 정책이 실기에서
전부 앞으로 넘어진 것과 관련이 있을 수 있다. 이것이 v86 을 대조군으로 넣은 이유다.

### 밤사이 고친 도구 버그

1. **학습과 재생의 망 크기가 달랐다** (`850782e`). 학습은 gym 등록의 러너(512-256-128)를
   쓰는데, 측정·재생은 `task_registry._BIG_NET_TASKS` 로 러너를 고른다. v83 ~ v87 이
   그 목록에 없어서 재생 쪽이 256-128-64 망을 만들었고, 체크포인트 로드가 실패했다.
   `tests/test_runner_consistency.py` 가 두 경로를 대조해 이런 불일치를 잡는다.
2. **"Isaac 이 떠 있는지"를 잘못 판정했다** (`f5533ca`). 명령줄 글자로 찾았더니, 그
   글자를 인자로 품은 셸도 Isaac 으로 잡혔다. 그 탓에 v86 학습이 끝난 뒤 큐가 16 분
   동안 멈춰 있었다. 이제는 `pgrep -x python3` 로 찾은 프로세스의
   `/proc/<pid>/cmdline` 에 `_isaaclab_launch.py` 가 있는지로 판정한다.

### 남은 것

1. **실기 A/B** — v85 / v86 / v87 을 같은 조건으로 돌린다. ONNX 는 각 런의
   `exported/` 에 준비돼 있다. 젯슨이 오프라인이라 아직 올리지 못했다
2. 젯슨에 `--lean-back` 이 들어간 `rl_walk.py` 올리기. 올리기 전에 젯슨 사본과
   리포 사본을 diff 한다
3. 실물 CoM 실측 (연필이나 판 위에 올려 균형점을 찾는 방법)
4. v83 측정. v84 는 v85 ~ v87 이 대신하므로 이어서 학습하지 않는다

### 추가: v88 — 앞 25 mm (같은 날 오후)

사용자 지시에 따라 v86 과 조건을 모두 같게 두고 몸통 CoM 만 앞으로 25 mm 옮겼다
(전체 무게중심 약 11 mm). 런은 `2026-09-17_09-54-06_imitation_v88`, 3000 iter.
학습 곡선은 계속 v86·v87 보다 낮았다 (iter 1316 에서 리워드 378 / 에피 659,
v86 은 427 / 748). 더 버티기 어려운 조건이니 예상대로다.

                 v87 (기준)            v86 (앞 15)           v88 (앞 25)
    일률 W       9.84 (9.67~9.93)      10.77 (10.72~10.79)   10.19 (10.09~10.30)
    포화 %       1.32 (1.28~1.40)      1.52 (1.49~1.57)      1.45 (1.39~1.56)
    rollRMS °    4.85 (3.82~6.14)      4.90 (4.27~5.38)      4.61 (4.14~5.10)
    roll 속      84.6 (66~112)         88.0 (64~104)         77.6 (64~96)
    낙상 %       0.73 (0.13~1.38)      0.90 (0.75~1.00)      0.54 (0.06~0.94)
    전후 추종    0.0160                0.0160                0.0169
    회전 추종    0.0424 (0.026~0.063)  0.0495 (0.032~0.072)  0.0172 (0.012~0.021)
    횡 추종      0.0377 (0.033~0.048)  0.0310 (0.027~0.034)  0.0426 (0.038~0.049)
    6 방향 부호  OK                    OK                    OK

범위가 겹치지 않는 차이만 추리면 다음과 같다.

- v88 과 v87 비교: 회전 추종 개선, 일률 소폭 증가
- v88 과 v86 비교: 회전 추종 개선, 일률 감소, 횡 추종 악화

회전 추종은 측정 잡음이 가장 큰 지표다 (반복 측정 범위/평균 223 %). 3 회 측정만으로는
이 개선을 확정할 수 없다. 앞의 결론도 그대로다 — 심 표로는 방향을 못 정하고, 실기
A/B 에 v88 을 넣어 판정한다. ONNX 검증 오차 4.17e-07. 녹화 `~/odm_out/video/v88_6dir.mp4`.
데스크톱이 젯슨과 다른 와이파이에 있어서 젯슨에는 아직 올리지 못했다.

### 추가: v88 실기 결과와 v89 / v90 — 전진·후진 정책 분리 (같은 날 저녁)

**v88 실기 (사용자)**

- 액션 LPF α=0.7 에서 가장 잘 걸었다
- 전진은 꽤 잘 걷는데 **후진하면 뒤로 넘어진다**
- 스로틀을 계속 당기면 **조금씩 옆으로 샌다**
- 회전은 되지만 잘 안 된다

**원인 후보 (실기 로그로 아직 확인하지 않았다)**

1. 옆 샘 — v65 계열은 path frame 의 **횡방향 오차**를 관측·리워드로 받으며
   학습됐다. 실기에는 위치 추정이 없어 `rl_walk` 가 늘 0 을 넣는다. 심에서
   옆 샘을 바로잡던 신호가 실기에는 없다. 자이로 바이어스는 08-18 에 정지
   상태에서 ~0 으로 쟀으므로 먼저 의심하지 않는다.
2. 약한 회전 — `--path-imu` 의 방위 오차는 회전 **명령**을 적분한 값과 비교한다.
   08-14 로그에서 실제 회전은 명령의 45 ~ 52 % 였다. 이 비율이면 오차가 계속
   쌓여 약 6 초 뒤 ±π 를 넘고, 부호가 뒤집혀 정책이 반대로 돌려 한다.

`scripts/diag/yaw_report.py` 가 실기 로그에서 두 가지를 가른다 (전진 중 자이로
적분 방향 변화, 회전 비율·오차 최대·±π 부호 뒤집힘 횟수).

**조치**

- `path_lateral_obs_zero` (env): 횡방향 칸을 남긴 채 0 을 넣고 리워드에서 뺀다.
  실기와 같은 값이고 관측 107 차원 그대로다
- v89 **전진 전용**: vx 0 ~ +0.15, CoM +30 mm, 횡방향 0, LPF 0.7 학습 포함
- v90 **후진 전용**: vx -0.15 ~ 0, CoM +15 mm, 나머지는 v89 와 같다.
  15 mm 는 v65(0, 앞으로 넘어짐)와 v88(+25, 후진에서 뒤로 넘어짐) 사이로 골랐다
- `rl_walk --onnx-back`: vx < -0.02 에서 후진 정책, vx > -0.005 에서 복귀
  (히스테리시스). READY·스케일·LPF 가 다르면 실행을 거부한다
- `rl_walk --path-yaw-clip RAD`: 방위 오차를 ±RAD 로 묶는다 (기본 끔)

한 판에 네 가지를 바꿨으므로 어느 것이 효과를 냈는지는 가를 수 없다.

**측정** (방향별 3 회 평균, 심)

    방향      지표              v87      v88      v89 전진   v90 후진
    전진      vx (명령 +0.15)   +0.149   +0.136   +0.144     (+0.050)
              일률 W / 포화 %   9.9/1.30 10.6/1.64 8.3/0.80  -
              옆 샘 vy          -0.002   +0.003   -0.010     -
              몸통 pitch °      +5.8     +5.3     +6.8       -
    후진      vx (명령 -0.15)   -0.127   -0.135   (-0.086)   -0.131
              일률 W / 포화 %   10.2/1.58 10.8/1.56 -        6.1/0.25
              roll RMS °        5.25     4.30     -          2.22
              몸통 pitch °      -3.6     -5.0     -          -7.1
    좌 / 우   vy (명령 ±0.20)   .159/.172 .158/.164 .139/.147 -
    회전      wz (명령 1.00)    0.958    0.983    0.941      -

괄호는 그 정책이 맡지 않는 방향이다. 쌍으로 쓰면 정지·좌우·회전은 v89 가 맡는다.

- v89 전진: 속도는 v88 보다 빠르고 일률·포화는 크게 줄었다. 옆 샘은 심에서 오히려
  늘었다 (-0.010 m/s) — 횡방향 신호 없이 걷기 때문이고, 이것이 실기와 같은 조건의 값이다
- v90 후진: 속도는 v88 과 같고 roll 흔들림은 절반, 일률은 절반 아래다. 다만 몸통을
  v88 보다 더 뒤로 젖힌다 (-7.1° 대 -5.0°). 실기에서 여전히 뒤로 넘어지면 v90 의
  CoM 을 +20 ~ +25 mm 로 옮겨 다시 학습한다
- 정지 중 넘어짐: v89 는 3 회 중 2 회, 환경 1 대가 6 ~ 9 초에 넘어졌다. v65 도 같은
  빈도라 v89 만의 문제는 아니지만 실기에서 정지할 때 주의한다
- v90 의 `obey` 실패(forward.vx)는 전진을 학습하지 않은 정책이라 예상된 결과다
- ONNX 검증 오차 v89 5.2e-07, v90 3.0e-07. 두 meta 는 런 이름만 다르다
  (`--onnx-back` 호환 검사 통과). 녹화 `~/odm_out/video/v89_6dir.mp4`, `v90_6dir.mp4`

**실기 실행**

    python3 ~/rl_walk.py --onnx ~/v89/policy.onnx --onnx-back ~/v90/policy.onnx \
        --path-imu --path-yaw-clip 0.5 --joy

LPF 는 meta 에서 0.7 이 자동으로 들어간다. 실행 뒤 `~/rl_walk_log.csv` 를
`yaw_report.py` 로 본다.

## 2026-09-18 — 미끄러운 바닥으로 학습(v91/v92)과 **같은 바닥에서 재기**

사용자: "88 번 할 때 조금씩 미끄러지는 것 같다. 심에서 마찰계수를 더 낮게 해서
학습 돌려봐 88 번 89 번." 처음 0.25~0.8 로 시작했다가 지시대로 **0.25~0.6 으로
처음부터 다시** 돌렸다 (1120 iter 돌던 런은 `*_imitation_v91_aborted_fric025_08`
로 이름만 바꿔 남겼다).

    v91 = v88 + 마찰 0.5~1.0 -> 0.25~0.6   (나머지 동일)
    v92 = v89 + 마찰 0.5~1.0 -> 0.25~0.6   (나머지 동일, 전진 전용)

### 함정: 정책마다 자기 바닥에서 재면 비교가 안 된다

처음 집계에서 v91 이 v88 보다 더 미끄러진 것으로 나왔다. 당연하다 — v91 은
미끄러운 바닥에서 쟀고 v88 은 원래 바닥에서 쟀다. "미끄러운 바닥에서 어느 쪽이
나은가" 는 **같은 바닥**에서 재야 한다. 그래서 측정 때 마찰을 덮어쓰는
`odm measure <ver> --fric 0.25,0.6` (env `ODM_FRIC`)을 만들어 교차 측정했다.
적용 지점은 `task_registry.env_cfg_for` 한 곳이다 (ODM_LPF 와 같은 방식).

### 같은 바닥 비교 (방향별 3 회 평균)

미끄러짐 = 발이 땅에 닿아 있는 동안의 발 수평속도 평균 (m/s).

    미끄러운 바닥 0.25~0.6      미끄러짐   전진vx  회전wz  낙상%   일률W
      v88 (보통 학습)            0.091     +0.147  0.960   0.46   10.12
      v91 (미끄럼 학습)          0.115     +0.140  0.965   0.23    9.51
      v89 (보통 학습)            0.077     +0.153  0.827   0.12    7.27
      v92 (미끄럼 학습)          0.066     +0.146  0.971   0.29    6.68

    보통 바닥 0.5~1.0
      v88                        0.061     +0.136  0.983   0.52   10.19
      v91                        0.091     +0.139  0.973   0.48    9.65
      v89                        0.041     +0.144  0.941   0.67    7.26
      v92                        0.049     +0.140  0.946   0.58    6.96

- **v92 는 v89 보다 낫다.** 미끄러운 바닥에서 v89 는 회전이 0.827 까지 무너지는데
  v92 는 0.971 을 지킨다. 미끄러짐도 더 적고(0.066 대 0.077) 일률도 낮다. 보통
  바닥에서도 손해가 거의 없다
- **v91 은 v88 보다 낫다고 말하기 어렵다.** 미끄러짐이 오히려 크고(0.115 대 0.091)
  낙상만 줄었다. 회전·속도는 사실상 같다
- 마찰을 낮춰 학습하면 **몸통을 덜 숙인다**: 전진 중 v88 +5.3° -> v91 +4.1°,
  v89 +6.8° -> v92 +2.5° (보통 바닥 기준)

### 회전 중 뒤로 넘어지는 문제 (실기)

사용자: "89 는 전진은 잘되는데 회전할 때 무게중심이 뒤로 가며 뒤로 넘어지고
후진은 안 된다." 후진은 설계상 v90 담당이라 `--onnx-back` 없이 돌리면 안 되는
것이 맞다. 회전은 심에도 같은 경향이 있다 — 회전 중 몸통 기울기:

      바닥           v88     v91     v89     v92
      미끄럼        -0.42   -3.71   -5.56   -1.89
      보통          +0.96   -2.39   -1.64   -0.04

v89 는 미끄러운 바닥에서 회전할 때 5.6° 나 뒤로 젖힌다. v92 는 1.9° 로 줄었다.
실기 바닥이 미끄럽다면 v92 가 회전 문제까지 같이 줄여 줄 후보다.

### 로그 보관

v89 실기 로그를 다음 실행이 덮어써서 잃었다. `rl_walk` 가 이제 직전 로그를
`~/rl_logs/<시작시각>_<정책>.csv` 로 옮긴다 (최근 20 개 유지).
