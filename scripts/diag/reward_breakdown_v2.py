...
Built directly on contact_diagnostic.py's skeleton (proven stable across
many runs) instead of the earlier reward_breakdown.py lineage, which
crashed silently on every attempt (6/6) for unexplained reasons even after
fixing a real cfg-selection bug -- this file avoids whatever that lineage
...
for step in range(args_cli.num_steps):
    ...
    raw_sum_step = torch.sum(torch.stack(list(terms.values())), dim=0)
    scaled_step = raw_sum_step * unwrapped.step_dt
    n_clamped += (scaled_step < 0.0).sum().item()
    clamp_loss_sum += torch.clamp(-scaled_step, min=0.0).sum().item()

    for k, v in terms.items():
        sums.setdefault(k, torch.zeros(args_cli.num_envs, device=unwrapped.device))
        sums[k] += v * unwrapped.step_dt
    sums.setdefault("__TOTAL__", torch.zeros(args_cli.num_envs, device=unwrapped.device))
    sums["__TOTAL__"] += scaled_step

    if step % 30 == 0:
        print(f"[step {step:3d}] alive of run", flush=True)

print("\n" + "=" * 70)
print(f"PER-TERM MEAN CONTRIBUTION over {args_cli.num_steps} steps, {args_cli.num_envs} envs")
print("=" * 70)
...
total = sums["__TOTAL__"]
print(f"  {'SUM (pre-clamp)':18s} sum={total.mean().item():+9.4f}   per-step-avg={total.mean().item()/args_cli.num_steps:+.5f}")
print("-" * 70)
