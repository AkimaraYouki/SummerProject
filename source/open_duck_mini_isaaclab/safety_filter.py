...
class ClearanceFilter:
    ...
    def __init__(self, ckpt: str, device, margin_mm: float = 5.0, alpha: float = 1.0):
        ...
        self.net = Barrier().to(device).eval()
    ...
    def _h(self, q5: torch.Tensor):
        """h(q) 와 그 그래디언트. 단위는 mm."""
        x = ((q5 - self.lo) / (self.hi - self.lo) * 2 - 1).requires_grad_(True)
        with torch.enable_grad():
            val = self.net(x) - self.bias - self.margin
            (grad,) = torch.autograd.grad(val.sum(), x)
        # 정규화 좌표에서 관절 좌표로 되돌린다
        return val.detach(), grad * 2.0 / (self.hi - self.lo)
