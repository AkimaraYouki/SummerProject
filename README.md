...
**최고 정책: `v34c10`** — 새 CAD(2.7140 kg) + 보행 궤적 `ref_g125`(몸통 139 mm)
+ 정지 시 보행 위상 고정. 정지 0.0058 m/s · 추종 0.0166 m/s 로 **둘 다 역대 최고**
(이전 최고 v28 의 추종 0.0171 을 처음 넘었다). 자세한 실험 경위는
[`docs/training_log.md`](docs/training_log.md) 의 v34 계열 항목.

| | 상태 |
|---|---|
| **로봇 모델** | ✅ 새 CAD 임포트 완료 — 2.7140 kg (CAD 표시 2.7152 대비 +1.2 g), 경고·오류 0 |
| **보행 궤적** | ✅ `ref_g125` — 105~135 mm 를 스윕해 고른 값. +10 mm 가 최적이고 +20 mm 는 오히려 나빴다 |
| **정지 안정성** | ✅ 정지 명령에서 위상을 0 에 묶어 0.0192 -> 0.0058 m/s |
| **실기 관절** | ✅ 14축 ID·영점·부호 확정 (`hardware_map.py`), READY 자세 실기 검증 통과 |
| **실기 IMU** | ✅ BNO055 축맵 항등 확정 (`imu_map.py`) — CAD·meshcat·실측 셋이 일치 |
| **실기 보행 재생** | ✅ 개루프 재생 가능 (`scripts/hw/play_ref_gait.py`) |
| **실기 통신** | ❌ **10.8 Hz. 정책은 50 Hz 필요** ← 지금 가장 큰 벽 |
| **발 접촉 센서** | ❌ 관측 2차원인데 실기 대응물 미확인 |
| **자가충돌** | ✅ 5 mm 기준, 6방향 전부 위반 0.0% · 접촉 0.0% |

**다음에 할 일**
1. 실기 통신 50 Hz — `latency_timer` 1 로, 보드레이트 1 Mbps 로 (`docs/handoff/project_hardware_bringup_2026-08-06.md` §4)
2. `right_hip_pitch` 가 레퍼런스에서 URDF 한계를 4° 넘는다. 높이로는 안 풀린다 —
   실기 가동범위를 재보거나(±45° 클램프가 보수적일 수 있다), 명령 격자 코너를 자르거나, 리셋 clamp
3. 발 접촉 센서 대응물 결정
4. ONNX 내보내기 + 온보드 통합
