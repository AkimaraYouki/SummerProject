...
**진행 중: `v33n` 학습** — 새 CAD 모델(2.7140 kg) + 새 보행 궤적(`ref_g115`) 으로
처음부터. 3000 iter, 4096 env. 텐서보드에 v26~v33n 이 겹쳐서 그려진다.

| | 상태 |
|---|---|
| **로봇 모델** | ✅ 새 CAD 임포트 완료 — 2.7140 kg (CAD 표시 2.7152 대비 +1.2 g), 경고·오류 0 |
| **보행 궤적** | ✅ `ref_g115` — 몸통바닥 지상고 114.9 mm. 105~125 mm 를 5 mm 격자로 스윕해 고른 값 |
